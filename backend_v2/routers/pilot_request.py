...
logger = logging.getLogger("the13th.backend_v2.routers.pilot_request")
...
def _build_problem_notes(payload: PilotRequest) -> str:
    """
    Condense several marketing fields into a single admin-facing notes string.
    """
    pieces = []
...
def create_pilot_request(
    ...
    """
    Create a Pilot from the marketing /pilot form.

    - Maps marketing site fields into the Pilot model used by /admin/pilots
    - Sets status=REQUESTED
    - Returns a simple JSON success payload for the frontend
    """
    try:
        logger.info(
            "Creating pilot for brokerage=%s, contact=%s <%s>",
        ...
        logger.info("Pilot created with id=%s", pilot.id)
        ...
        logger.exception("Error while creating pilot request: %s", exc)
        raise HTTPException(
